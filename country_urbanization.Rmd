...
plot_top <- df_reshaped_nig %>% 
  filter(City %in% c("Lagos", "Abuja", "Port Harcout", "Kano", "Ibadan")) %>% 
...
# Add MSPE value on top of one bar
plot2 + 
...
newdf2cons <- newdf2 %>%
  filter(ccode %in% c("NGA","ZAF", "KEN") & year %in% c(2022, 2030) & !name %in%  c("Residual_KEN", "Residual_ZAF", "Residual_NGA")) %>%
  group_by(ccode, name, year) %>%
  mutate(spending_threshold_group = ifelse(spending_threshold >= 120, "over 120",
                                            ifelse(spending_threshold >=80, "over 80",
                                               ifelse(spending_threshold >= 40, "over 40",
...
for (i in 1:(length(buckets) - 1)) {
  ...
  total_spending <- sum(df2$tot_cons)
...
```{r}
## Get consumer class distribution from survey
...
for (county in unique(ken_cons_ppp$county)) {
  # Filter the dataframe for the current county
    ...
    for (i in 1:(length(buckets) - 1)) {
      ...
      total_spending <- sum(filtered_data$tot_cons)
...
plot40 <- kenya_cities %>% 
  ...
  ggplot(aes(x = reorder(City, -Value), y = Value, fill = Method)) + 
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "City", y = "$12+ Consumer class", title = "$40+ consumer class in Kenya") +
  theme_minimal() +
  coord_flip() +
  theme(axis.title.x = element_text(size =  10), 
        axis.title.y = element_text(size = 10),
...
plot80 <- kenya_cities %>% 
  ...
  ggplot(aes(x = reorder(City, -Value), y = Value, fill = Method)) + 
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "City", y = "$12+ Consumer class", title = "$80+ consumer class in Kenya") +
  theme_minimal() +
  coord_flip() +
  theme(axis.title.x = element_text(size =  10), 
        axis.title.y = element_text(size = 10),
