...
WDPro <- "/Users/lindakarani/Library/CloudStorage/GoogleDrive-linda.karani@worlddata.io/Shared drives/DATA_WDL/Mpro_2.0/01_Data/R_2023_04_11/2023_04_26_ukr_rebase_2017ppp/03_outputs"
...
##World data pro
worlddatapro <- read_rds(paste0(WDPro, "/05_003_merge_mpro2_ages_R_2023_04_11_2017ppp_1_USD.rds"))

## ccodes and regions

region <- read_csv("ccodes.csv")

region <- region %>% 
    dplyr::select(`alpha-3`, region, `sub-region`) %>% 
    rename(ccode = `alpha-3`)

worlddatapro_2023 <- worlddatapro %>% 
    filter(year %in% c("2021","2022","2023"))
    

##Merge worlddata pro with regions data set

worlddatapro2123 <- merge(worlddatapro_2023, region, by = c("ccode"))
...
```{r}
##Get rural/urban classification from wdpro

cc_all <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023") & inc_lwr >= 12.00) %>%
    group_by(year) %>% 
    summarise(pop = sum(hc))


##Get rural/urban classification from wdpro by region

cc <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023") & inc_lwr >= 12.00) %>%
    group_by(year, region) %>% 
    summarise(pop = sum(hc)) %>% 
    mutate(pop_urban = pop*0.85)

cc_agg <- cc %>% 
    group_by(year) %>% 
    summarise(tot1 = sum(pop),
              tot2= sum(pop_urban))

cc_long <- cc %>% 
    pivot_wider(names_from = "year", values_from = c("pop", "pop_urban")) %>% 
    mutate(diff = `pop_urban_2023` - `pop_urban_2021`)

cc_sum <- cc_long %>% 
    summarise(total = sum(diff))

poor <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023") & inc_lwr < 12.00) %>%
    group_by(year, region) %>% 
    summarise(pop = sum(hc))

all <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023")) %>%
    group_by(year) %>% 
    summarise(pop = sum(hc))

##Get rural/urban classification from wdpro by subregion

cc_sub <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023") & inc_lwr >= 12.00) %>%
...
poor_sub <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023") & inc_lwr < 12.00) %>%
    group_by(year, `sub-region`) %>% 
    summarise(pop = sum(hc))

all_sub <- worlddatapro2123 %>% 
    filter(year %in% c("2021","2022","2023")) %>%
    group_by(year, `sub-region`) %>% 
    summarise(pop = sum(hc))
...
eth <- worlddatapro %>% 
    filter(ccode == "ETH" & year %in% c("2015", "2016" ,"2023") & inc_lwr >= 15.00) %>%
    group_by(year) %>% 
    summarise(pop = sum(hc))

eth_pop <- worlddatapro %>% 
    filter(ccode == "ETH" & year %in% c("2015", "2016")) %>%
    group_by(year) %>% 
    summarise(pop = sum(hc))
...
colgate <- resid_cit %>% 
    unnest(cols = spending_data_datt) %>% 
    filter(ccode %in% c("KEN", "COD", "ETH", "NGA", "AGO"))

# add wdp spending levels
colgate$spend_group <- cut(colgate$spending_threshold, breaks = c(0, 12, 40, 80, 120, Inf), right = FALSE)

# sum up weights and spending by spending levels
colgatefin <- colgate %>%
    filter(year %in% c(2023, 2030)) %>% 
    group_by(ccode, year, name, spend_group) %>%
    summarise(hc = round(sum(headcount)),
              exp = round(sum(spending))) %>%
    ungroup() %>% 
    write_csv("africa_cities_check.csv")
...
```{r}
## Get the totals for the three countries without including upper bounds i.e over 12, over 40 and over 80 (slides 8 and 9) - Nightlights actual numbers
resultsum <- result %>% 
  group_by(ccode, year) %>%
...
##Read in World data pro numbers
...
```{r}
## Get spending distribution from nightlights rescaled numbers - slides 8 and 9


options(scipen = 999)
# get totals for different consumer classes 

newdf2cons <- newdf2 %>%
  filter(ccode %in% c("KEN", "NGA", "ZAF") & year %in% c(2022, 2030) & !name %in%  c("Residual_KEN", "Residual_ZAF", "Residual_NGA")) %>%
  group_by(ccode, name, year) %>%
  mutate(spending_threshold_group = ifelse(spending_threshold >= 120, "over 120",
                                            ifelse(spending_threshold >=80, "over 80",
                                               ifelse(spending_threshold >= 40, "over 40",
...
## Sum for nightlights rescaled values for $12+, $40+ and $80+
newdf2cons_sum <- newdf2cons %>% 
  group_by(year, ccode) %>%
  summarise(below12 = sum(headcount_sum[spending_threshold_group %in% c("below 12")]),
            over12 = sum(headcount_sum[spending_threshold_group %in% c("over 12", "over 40", "over 80", "over 120")]),
            over40 = sum(headcount_sum[spending_threshold_group %in% c("over 40", "over 80", "over 120")]),
...
## Include upper limits. For some reason the result is the same with the code chunk above
survey_df2 <- df %>%
  mutate(spending_threshold_group = ifelse(inc_hgr >= 120 , "over 120",
...
survey_tot <- ken_cons_ppp %>% 
  summarise(pop = sum(weight_pop),
            hhspendppp = sum(tot_cons)/38.344658,
            individualppp = sum(tot_cons_ppp))
```
...
## Create spending groups with Kenyan survey
...
## Get the residual from the Kenyan survey i.e everything minus Nairobi and Mombasa
...
```{r}
## Get totals for the key cities in Kenya i.e $12 +, $40+ and $80+
...
## Get totals for the residual in Kenya i.e $12 +, $40+ and $80+
...
```{r}
## Get residual to match kenya survey i.e filter for everything except Nairobi and Mombasa to compare to residual from Kenya survey

cons_ken <- newdf2 %>% #rescaled nighlight data frame
  filter(ccode %in% c("KEN") & year == "2016" & !name %in% c("Nairobi", "Mombasa")) %>%
  group_by(ccode, name, year) %>%
  mutate(spending_threshold_group = ifelse(spending_threshold >= 120, "over 120",
                                            ifelse(spending_threshold >=80, "over 80",
                                               ifelse(spending_threshold >= 40, "over 40",
...
## Comparison with survey data(plots to validate with Kenyan data)
